...
const binanceSignal = require('./helpers/binanceTrade');
...
app.use(
  expressValidator({
    customValidators: {
      ...
      validAmount(amount, wallet, user) {
...
app.use((req, res, next) => {
  const allowLevel4Pages = ['/dashboard', '/logout', '/', '/account/createAccountOverflow'];
  if (req.user) {
...
app.get(
  ...
  passportConfig.isAuthenticated,
  NavigationMiddleware,
  userController.getMyLink
); // link ref
app.get(
  '/account/listuserfloor',
  passportConfig.isAuthenticated,
  NavigationMiddleware,
  userController.getlistuserfloor
); // link list user of floor
...
app.post(
  '/account/requestWithdrawn',
  passportConfig.isAuthenticated,
  walletController.postRequestWithdrawn
);
app.post('/account/transferBtc', passportConfig.isAuthenticated, walletController.postTransferBtc);

app.get(
  '/account/reqWithdrawnList',
  passportConfig.isAuthenticated,
  NavigationMiddleware,
  userController.getReqWithdrawnList
);

app.get(
  '/account/withdrawn/checkDone',
  passportConfig.isAuthenticated,
  userController.withdrawnCheckDone
);
...
app.post(
  '/account/changeBlockChain',
  passportConfig.isAuthenticated,
  userController.postChangeBlockChain
);
...
app.get(
  ...
  passportConfig.isAuthenticated,
  NavigationMiddleware,
  listCoinController.getReqWithdrawnList
);
app.get(
  '/admin/configbittrex',
  passportConfig.isAuthenticated,
  NavigationMiddleware,
  configBittrexController.index
);
